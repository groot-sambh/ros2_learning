...
#include "my_robot_interfaces/msg/detail/led_state_array__rosidl_typesupport_fastrtps_c.h"
...
using _LedStateArray__ros_msg_type = my_robot_interfaces__msg__LedStateArray;
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: led_state
  {
    ...
    if (!rosidl_runtime_c__int64__Sequence__init(&ros_message->led_state, size)) {
      fprintf(stderr, "failed to create array for field 'led_state'");
      return false;
    }
    auto array_ptr = ros_message->led_state.data;
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = my_robot_interfaces__msg__LedStateArray;
    is_plain =
      (
...
static size_t _LedStateArray__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_my_robot_interfaces__msg__LedStateArray(
